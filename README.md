...
> **这句"不需要公网 IP"指的是内网那台跑客户端的机器**——它主动向外拨号，所以不用公网 IP、
> 不用改路由器、不用做端口映射。**服务端那台则必须有公网可达地址**（VPS / 有公网 IP 的宽带 /
> 有公网映射的设备），否则访客无从连入；两台机器都在纯内网时，本工具无法凭空造出一条公网路径。
>
> 顺带一句判断标准：如果两台机器本来就在同一局域网内、能直接互通，**直接访问内网地址就行**，
> 绕本工具一圈只是让流量多跑一趟公网，更慢、更脆，也多一份对外暴露面。
...
- **服务端管理台**：`server_gui.py` 看得到**在线客户端**（身份 / 对端地址 / 认领的内网端口 /
  ...
  同进程、不动协议，服务端从此不再是黑盒
- **管理台踢人**：急着赶走一台卡死／乱来的机器时，在在线表里选中它踢出去——控制连接立刻断开、
  端口归属立即释放，并在冷却期（`admin.kick_cooldown`，默认 30 秒）内拒绝它重新注册，
  冷却结束后它自己回来。确认框先把身份、对端、认领端口与后果列清，防误点
...
上面这一套是**本机自环演示**（服务端与客户端都在本机 `127.0.0.1`）。要让**公网访客**访问，
必须换成两台机器：

```bash
# 公网那台（VPS 等公网可达机器）：起服务端，并把数据通道地址下发给客户端
python server.py --advertise-host <公网IP> --auth-file tokens.json

# 内网那台（要暴露服务的那台）：客户端主动向外拨号，不需公网 IP、不需改路由器
python client.py --server <公网IP>:7000 --local-ports 8000 --client-id <身份> --token <令牌>
```

访客随后访问 `http://<公网IP>:9028/`。注意两点：

- **「不需要公网 IP」说的是跑客户端的那台**；服务端那台必须有公网可达地址，否则访客无从连入。
- `--advertise-host` 是**唯一**能改变"客户端该回连哪个地址"的开关。不填时服务端会复用客户端
  连入时看到的本地地址——本机演示没问题，跨公网部署就会让客户端去连一个连不通的地址。
...
环境变量也只覆盖 `control/data` 端口与 `auth`、`mapping_store`、`tls`、`admin`）。
要调限流参数请改 `config.json`。
...
急着赶走一台卡死、行为异常、或者干脆不该在线的机器时，不必停服务端、更不必重启它：

```text
在线客户端表（选中一行）→ [踢出选中客户端] → 确认框 → 断开 + 冷却期
```

**踢出做了什么**（服务端一口气做完，不经过协议）：

| 动作 | 说明 |
| --- | --- |
| 控制连接断开 | 正在通过它转发的请求会失败（这是踢人的本意） |
| 端口归属释放 | 它认领的内网端口立刻可以被别的客户端抢占 |
| 挂起请求收尾 | 排队等它配对的访客立刻收到失败，不会挂到超时 |
| 限速桶回收 | 按归属清掉它的全部分片桶（不是前缀删，见「桶表回收与上限」） |
| 事件与日志 | 复用 `CLIENT_DISCONNECTED`，`reason` 写明"被管理台踢出" |
| **冷却期** | `admin.kick_cooldown` 秒内拒绝该 `client_id` 重新注册 |

### 冷却期：为什么必须要有

不做冷却的"踢人"基本等于没踢——客户端自己的重连退避初始延迟就是个位数秒级，
刚断掉它下一轮就回来了。所以踢出后服务端会记一段冷却期：

```json
{ "admin": { "kick_cooldown": 30 } }
```

| 字段 | 默认 | 说明 |
| --- | --- | --- |
| `kick_cooldown` | `30`（秒） | 被踢出的 `client_id` 在这么多秒内拒绝重新注册；`0` ＝不冷却（"只断一下"） |

几条刻意定下来的语义：

- **拒绝走 `403 + retryable=true`**，与"端口未授权"同一类：**暂时**不行、等会儿再来。
  客户端按退避策略继续重试，冷却一过**自动上车**，不需要人工重启那台机器。
  刻意**不用** `503`（那在本项目里是"服务端整体没位置"，会把"这台机器被踢了"
  说成"服务器满了"），也不用 `429`（那是单客户端配额）。
- **冷却期判定排在鉴权之后、容量之前**。排在鉴权之前，"令牌已失效"会被渲染成
  "你在冷却期"，排查方向直接被指错；而"你被踢了"是针对这一台的，
  比"整机容量满"更具体，所以它先报。
- **只有管理台踢人才登记冷却期**。被顶号、心跳超时、控制连接断开都不登记——
  那些是"这条连接没了"，不是"这台机器被要求离开一会儿"；给它们加冷却等于
  "客户端网络抖一下就再也上不来"，与看门狗的意图正好相反。
- **冷却期看得见**：`snapshot()["kick_cooldowns"]` 是 `{client_id: 剩余秒数}`，
  管理台状态栏在非空时显示「踢出冷却中 N 台（剩 M 秒）」。
  冷却中的机器**不在**在线表里，没有这一格，界面上只表现为"那台机器一直没上来"。
- **只留 JSON 与环境变量两条路**（`LOCALTONET_ADMIN_KICK_COOLDOWN`），不做命令行开关——
  与 `limits`、`mapping[].tls` 同一先例：它描述的是"管理台点了踢人之后服务端怎么反应"，
  不是启动参数。

### 误点保护

这是管理台唯一**直接断人**的操作（改映射只影响之后的新请求，不会掐断已有隧道），
所以门槛刻意比"删除一行映射"更高一档：

- 按钮**必须先选中一行**才会亮，且服务端真的在跑；
- 二次确认框列出**身份、客户端 ID、对端地址、认领的端口**，
  并逐条说明后果（连接断开 / 端口释放 / 冷却期内无法重连）；
- 目标已经不在线时**如实告诉你**（"已经不在线，无需踢出"），不报错也不静默——
  界面每 0.5 秒采样一次，你看到的名单天生可能滞后一个周期。

### 边界：踢人不需要"权限模型"

在线表曾经被刻意做成只读，理由是"没有权限模型，一个误点的按钮就能掐断正在服务的隧道"。
这一条并不适用于此：**管理台是本机同进程的窗口**——能打开它的人本来就能停服务端、
改全局映射表（见「服务端管理台」）。权限边界是"本机同进程"这件事本身，
不需要再造一套模型；真正缺的一直是**误点保护 + 冷却期语义**，也就是上面两节。
反过来，这也意味着**踢人不是远程能力**：要远程踢人得先有远程管理（协议扩展 + 管理权限模型）。
...
| 区域 | 内容 |
| --- | --- |
| 在线客户端（选中可踢） | 身份、客户端 ID、对端地址、认领的内网端口、在线时长、空闲时长；空闲达到看门狗阈值会标「将失联」。底部有「踢出选中客户端」（见「管理台踢人」） |
| 映射表（可编辑） | 与客户端界面同一套增删改查与三态「访客 TLS」；提交后立即起停访客端口，**并广播给所有在线客户端** |
| 状态栏 | 服务端名、鉴权模式、在线数、监听端口、挂起通道、注册/被拒计数、请求与失败计数、上下行字节、限速口径（非默认时才显示）、限速累计等待、桶表淘汰数（非 0 时才显示）、踢出冷却期（非空时才显示） |
| 事件日志 | 启动/停止、客户端上线下线（含身份与原因）、踢出结果、转发失败原因、**映射表改动被拒（谁被拒、为什么）**、**注册被拒（谁、什么码、什么原因、会不会继续重试）**；请求完成刻意不记，见下 |
...
**在线客户端表有一个写操作：踢出选中客户端。** 它是这个窗口里唯一的**直接断人**动作
（改映射只影响之后的新请求），所以配了完整的误点保护（先选中 + 列出身份/对端/端口与后果的
确认框）与一段冷却期，细节见「管理台踢人」。判定与执行都在服务端
（`TunnelServer.kick_client` → 下线内核 `_disconnect_session`），界面只负责确认与投递。
...
| 管理台的断人动作 | `server.core.TunnelServer.kick_client` | 复用下线内核 `_disconnect_session`（顶号 / 心跳超时 / 踢人共用一份）+ 冷却期表 | 强制下线的审批流、按身份的限流与封禁、踢人审计落库 |
...
| 管理台踢人 | 端口归属**立即释放**、冷却期内以 `403 + retryable=true` 拒绝重连、冷却结束后**自动上车**（无需人 restart） |
| 踢不存在的客户端 | 是空操作，且**不留下冷却期**（界面每 0.5 秒采样一次，它可能刚自己掉线） |
| 冷却期为 0 | "只断一下"：不留任何冷却状态，客户端立刻能回来（走的是同一段下线内核） |
...
> 七期四组（管理台踢人）——拆掉冷却期判定（`remaining > 0` → `< 0`）→ 1 红
> （`10.0s 内冷却期拒绝重连未满足`）；`DEFAULT_KICK_COOLDOWN` 翻成 `0.0` → 1 红（`assert 0.0 == 30.0`）；
> `snapshot()` 里把键写成 `kick_cooldownz` → **4 红**（全部 `KeyError: 'kick_cooldowns'`）；
> 不在线也登记冷却期 → 1 红（`{'nobody-here': 30.0} == {}`）。
> 每组改回后都用 `grep -n` 逐行复核过关键行，并确认仓库里没有 `MUTATION` 残留
> （不靠 `cp` 回滚，见 PITFALLS C10/C11）。
...
踢人横跨两处接缝，所以两边各有一条用例，验的东西刻意不同：
`test_e2e.py` 验**判定**（端口真的释放 / 冷却期内真的被拒 / 冷却结束后真的自动上车）；
`test_gui_server_controller.py` 验**界面知道发生了什么**（`kick_result` 到达日志与提示、
`CLIENT_DISCONNECTED` 穿过转发白名单、状态栏里的冷却期、再踢一次时如实说"已经不在线"）。
写后者时踩到过一次本仓库的老坑：先到的是**事件**、后到的是**周期快照**，
直接断言状态栏会读到踢出之前那一份快照——等待条件必须覆盖你要断言的那一侧。
...
- 管理台**能踢人，但只能踢"本机看到的那个进程里的人"**：踢出的判定与执行全在服务端本进程
  （`TunnelServer.kick_client`），没有任何指令、没有审计落库、也没有"按身份封禁"——
  它回答的是"现在把这台机器请出去"，不是"以后都不许它来"
- 踢出后重新上车的**真实延迟是"冷却期 + 客户端当前那一段退避间隔"**，可能明显长于冷却期本身
  （默认退避 1s → 2s → … 最多 60s；退避序列只在注册成功时归零）。
  客户端在被拒期间并不知道冷却还剩多久，只能按自己的节奏重试。
  想让"冷却一过就回来"，把 `admin.kick_cooldown` 设得明显小于客户端的 `reconnect.max_delay`
- 冷却期只按 `client_id` 记账，**不做持久化**（重启服务端即清空），条目数有全局上限
  （超出时先清已过期的、再淘汰最早登记的）；也就是说它是"运行期的一段记忆"，
  不是一份持久化的黑名单
- 管理台**每 0.5 秒采样一次**状态，所以界面上的在线数与字节数最多滞后一个采样周期；
  要更实时得加推送而非提高采样率（采样率越高，事件循环线程被占用的时间越多）。
  同理：**在线表里选中一行点踢出时，那台机器可能刚好自己掉线了**——
  这种情况会得到一句"已经不在线，无需踢出"，而不是构造出一次"踢成功"的假象
...
后续计划：远程管理（需要协议扩展 + 管理权限模型）→ 限速值下沉到 `mapping[]`（每端口独立额度）
→ 映射表的多进程/多实例安全（需要跨平台文件锁与版本号语义，而本工具的设计就是单进程单服务端，
价值存疑）。

已完成：给映射表写操作加权限判定（四期）→ 按请求的带宽统计与限流粒度细化（六期：
按端口 / 按访客 IP 的限速口径 + 按端口的带宽统计 + 桶表回收与上限）→
管理台踢人（七期：选中踢出 + 端口立即释放 + 冷却期内拒绝重连 + 误点保护）。
